...
# macOS: ~/Library/Logs/ygs
# Linux: $XDG_STATE_HOME/ygs/logs or ~/.local/state/ygs/logs
if [ "$(uname)" = "Darwin" ]; then
    LOG_DIR="${HOME}/Library/Logs/ygs"
else
    LOG_DIR="${XDG_STATE_HOME:-${HOME}/.local/state}/ygs/logs"
fi
...
if [ -z "${NODE_EXEC}" ]; then
    {
        ...
        echo "To fix: Set CHROME_MCP_NODE_PATH environment variable or run 'ygs doctor --fix'"
    } >> "${WRAPPER_LOG}"
    exit 1
fi
