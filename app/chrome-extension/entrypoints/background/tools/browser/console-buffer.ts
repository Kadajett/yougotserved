...
/**
 * A rolling console log buffer.
 *
 * One buffer for each tab, filled as console events arrive.
 * A tab that moves to another domain starts a fresh buffer, so logs from two sites never mix.
 */
...
/**
 * Builds a preview from a CDP RemoteObject. objectId is dropped: holding it leaks memory.
 */
function extractArgPreview(arg: unknown): unknown {
  const a = arg as Record<string, unknown>;
  if (!a || typeof a !== 'object') return arg;

  // Keep the safe fields only.
...
class ConsoleBuffer {
  ...
  /**
   * True when this tab is being captured.
   */
  ...
  /**
   * Starts capture for this tab if it is not running.
   */
  ...
  private handleTabUpdated(
    ...
    if (typeof nextUrl === 'string') {
      ...
      // A new domain starts a fresh buffer.
  ...
  private handleDebuggerEvent(
    ...
    if (method === 'Runtime.consoleAPICalled' && p) {
      ...
      state.messages.push({
        ...
        // Store the preview only, so nothing leaks.
