<template>
  ...
  >
    ...
    <!-- Browser tools, only shown when Claude or Codex CLI is selected -->
    <div v-if="showChromeMcpOption" class="px-3 py-2 flex items-center gap-2">
      ...
      <label
        class="flex items-center gap-2 cursor-pointer"
        title="Let this agent drive the browser through the local bridge"
      >
        <input
          type="checkbox"
...
// Show the browser tools option when Claude, Codex, or Auto (empty) is selected
